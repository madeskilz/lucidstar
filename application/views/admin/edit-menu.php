    ...
    <div class="row">
        ...
        <div class="col-md-6">
            ...
            <ol id="menu-items-list" class="nested-sortable" style="margin-top:10px;">
                ...
                $tree = array(); $lookup = array();
                foreach ($items as $it) { $lookup[$it->id] = (array)$it + ['children'=>[]]; }
                foreach ($lookup as $id => &$node) {
                    $pid = intval($node['parent_id']);
                    if ($pid && isset($lookup[$pid])) { $lookup[$pid]['children'][] = &$node; } else { $tree[] = &$node; }
                }
                ...
                foreach ($tree as $n) { echo render_node($n); }
                ?>
            </ol>
        </div>
    </div>
...
;(function(){
    ...
    function initNested(){
        ...
        document.getElementById('save-order').addEventListener('click', function(){
            ...
            var payload = JSON.stringify(serialized.map(function(it){ return {id: it.item_id, parent_id: it.parent_id}; }));
    ...
    function tryLocalThenCdn(){
        ...
        var s1 = document.createElement('script'); s1.src = localUi;
        s1.onload = function(){
            var s2 = document.createElement('script'); s2.src = localNested;
